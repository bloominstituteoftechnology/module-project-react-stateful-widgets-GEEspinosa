...
export default function Programmers() {
  ...
  const [poop, pooping] = useState(null)
  ...
  const getNameOfFeatured = () => {
    ...
    programmers.forEach(programmer => {
      if(programmer.id === poop){celebrate = programmer.name}
    })
    return celebrate
  };
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { pooping(dev.id)/* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
